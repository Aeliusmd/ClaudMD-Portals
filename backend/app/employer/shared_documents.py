"""Resolve secure shared-document links via SharedDocuments.SharedId → DocumentUploads."""
...
from app.employer.employee_search import _format_display_date, _gender_label
...
def get_shared_document_detail(
    ...
    # Secure-link open counts as viewed for this SharedId only.
...
def _load_shared_document_access(
    current_user: CurrentUser,
    shared_id: str,
) -> tuple[Any, EmployerProfile, dict[str, Any]]:
...
def _mark_shared_id_viewed(
    ...
    """
    Mark one SharedDocuments row as viewed (IsViewed = 1).
    Scoped to the current recipient + SharedId — does not touch other shares.
    """
...
def _resolve_upload_pdf_path(row: dict[str, Any]) -> Path | None:
    """
    Use DocumentUploads.FilePath for the shared file.
    Supports a full file path or a folder + FileName.
    """
    ...
    try:
        ...
        if root.is_dir() and file_name:
            ...
            if candidate.is_file():
                return candidate
            # Case-insensitive match inside the folder.
...
def _patient_display_name(first_name: Any, last_name: Any) -> str:
    ...
    return " ".join(part for part in [first, last] if part).strip() or "Employee"
