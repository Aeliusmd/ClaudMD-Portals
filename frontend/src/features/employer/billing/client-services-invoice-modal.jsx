...
const InvoiceSheet = forwardRef(function InvoiceSheet({ invoice }, ref) {
  return (
    <div
      ref={ref}
      className="rounded-xl border border-border/70 bg-white px-4 py-5 shadow-sm sm:px-8 sm:py-7"
    >
      ...
      <div className="mb-5 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="min-w-[14rem] space-y-2">
          ...
          <div className="mt-2 rounded-md border border-border/80 bg-[#f3f3f3] px-3 py-2.5">
            <MetaRow label="Amount Due:" value={formatMoney(invoice.amountDue)} />
            <div className="mt-1">
      ...
      >
        <p>
          ...
          <span className="font-bold text-ink">{invoice.patientName || "—"}</span>
        </p>
        <p>
          <span className="font-semibold" style={{ color: ORANGE }}>
            SSN:{" "}
            ...
            {(invoice.lines || []).length === 0 ? (
              ...
              invoice.lines.map((line) => (
                <tr key={line.id} className="border-b border-border/50">
                  ...
                  <td className="px-2 py-2 text-ink">{line.description || "—"}</td>
...
export function ClientServicesInvoiceModal({
  open,
  invoice,
  loading,
  error,
  ...
  const downloadStartedRef = useRef(null);
  ...
  useEffect(() => {
    if (!downloadOnLoad) {
      downloadStartedRef.current = null;
      return;
    }
    if (!open || loading || error || !invoice || !invoiceRef.current) return;

    const downloadKey = `${invoice.billingHeaderId}-${invoice.historyId || "latest"}`;
    if (downloadStartedRef.current === downloadKey) return;
    downloadStartedRef.current = downloadKey;
    ...
    async function downloadInvoice() {
      try {
        ...
        const invoiceNumber = String(invoice.invoiceNumber || invoice.historyId || "invoice")
          .replace(/[^a-z0-9_-]+/gi, "-")
          .replace(/^-+|-+$/g, "");
        pdf.save(`invoice-${invoiceNumber || "billing"}.pdf`);
        onDownloadComplete?.(null);
      } catch (downloadError) {
        onDownloadComplete?.(
          downloadError instanceof Error
            ? downloadError
            : new Error("Unable to download invoice.")
        );
      }
    }
  ...
  // Download-only: render the sheet off-screen for capture, no visible viewer.
  if (downloadOnLoad) {
    ...
    return (
      <div
        ...
        style={{ left: "-10000px", width: "64rem" }}
      >
        <InvoiceSheet ref={invoiceRef} invoice={invoice} />
      </div>
    );
  }
