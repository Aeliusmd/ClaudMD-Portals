...
import {
  markAllInsuranceNotificationsRead,
  markInsuranceNotificationItemRead,
} from "@/hooks/use-insurance-notifications";
...
function NotificationsListSkeleton({ rows = PAGE_SIZE }) {
  return (
    <Card
...
export function InsuranceNotificationsView() {
  ...
  const markItemAsRead = useCallback((notification) => {
    ...
    markInsuranceNotificationItemRead(notification);
    setItems((prev) =>
      prev.map((item) =>
  ...
  const markAllAsRead = useCallback(async () => {
    setMarkingAll(true);
    try {
      await markAllInsuranceNotificationsRead();
      setItems((prev) =>
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-5">
      <button
        type="button"
        onClick={() => router.push(insurancePaths.dashboard)}
      ...
      {showInitialSkeleton ? (
        ...
        <EmptyState
          ...
          description="New shared reports, appointments, and work status updates for your insured patients will appear here."
        />
      ) : (
        <>
          {error ? (
          ...
          <Card className="overflow-hidden p-0 shadow-[0_8px_30px_rgba(28,36,48,0.06)]">
            ...
            {showPageSkeleton ? (
              ...
              >
                {Array.from({
                  length: Math.min(PAGE_SIZE, total || PAGE_SIZE),
                }).map((_, index) => (
