...
/**
 * Scoped Shared Documents view after insurance secure-link login.
 * Live path only: SharedDocuments.SharedId scoped to this insurance company.
 */
export function InsuranceScopedSharedDocumentsView() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const detail = await fetchInsuranceSharedDocumentBySharedId(
          token,
          active.sharedId
        );
        if (cancelled) return;
